...
function Client() {
  ...
  var contacts = {
    ...
    createContact: function(options, cb) {
        var call = {
          method: 'POST',
    ...
    getContactByEmail: function(options, cb) {
        if (typeof options === 'function') {
          cb = options;
          options = {};
        }
        var call = {
          method: 'GET',
          url: self.baseUrl + 'contacts/v1/contact/email/'+ options.contact_email + '/profile'
        };
        sendRequest(call, cb);
    }
  };
  ...
  var lists = {
    ...
    addContactToList: function(options, cb) {
        var call = {
          method: 'POST',
          url: self.baseUrl + '/contacts/v1/lists/' + options.list_id + '/add',
          options: options
        };
        sendRequest(call, cb);
    }
  ...
  function sendRequest (call, cb) {
    ...
    if(call.method === 'POST' ) {
        var formData = call.options || [];
        if (self.key) {
          call.url += '?hapikey=' + self.key;
        } else if (self.token) {
          call.url += '?hapikey=' + self.token;
        }
        request.post({url: call.url, json: formData }, handleResponse(cb));
    } else {
        call.qs = call.qs || {};
